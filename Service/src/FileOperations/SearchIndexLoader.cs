...
{
    public class SearchIndexLoader : ISearchIndexLoader
    {
        private readonly string folder;
        private LoadedIndex _index;
        private readonly IConfiguration _configuration;

        public SearchIndexLoader(IConfiguration config)
        {
            _configuration = config;
            folder = _configuration["AppSettings:IndexFolder"].TrimEnd('/', '\\') + Path.DirectorySeparatorChar;
        ...
        private void LoadDictionary()
        {
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                if(_index.Dictionary.TryGetValue(term, out var dictVal))
                {
                    // Create a new TermDictionaryEntry with updated DocFreq and OffsetList
                    var newEntry = dictVal with
                    {
                        OffsetList = new List<OffsetLengthEntry>(dictVal.OffsetList) { new OffsetLengthEntry(offset, length) },
                        DocFreq = dictVal.DocFreq + df
                    };
                    _index.Dictionary[term] = newEntry;
                }
                else
                {
                    List<OffsetLengthEntry> offsetList = new List<OffsetLengthEntry>();
                    offsetList.Add(new OffsetLengthEntry(offset, length));
                    _index.Dictionary[term] = new TermDictionaryEntry(offsetList, df);
                }
            }
        }
